...
outputpath = './scmaui_pbmc_multi_adv_v1'

datapaths = ['data/gtxcombined.h5ad', 'data/peaks.h5ad']

adatas = load_data(datapaths, ['gtx','peaks'])
dataset = SCMauiDataset(adatas, losses=['zinb', 'negmul'], union=True, adversarial=['sample'])
print(dataset)
...
ensemblevae.fit(dataset, epochs=300, batch_size=32)
