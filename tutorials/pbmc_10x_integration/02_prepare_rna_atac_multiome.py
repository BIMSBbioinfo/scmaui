...
pos = pd.DataFrame([x.replace(':','-').split('-') for x in interval], columns=['chrom','start','end'])


mat = csc_matrix((handle['matrix']['data'][:], handle['matrix']['indices'][:], handle['matrix']['indptr'][:]), shape=handle['matrix']['shape'][:]).T

barcodes = pd.DataFrame(index=_decode(handle['matrix']['barcodes'][:]))
features = pd.DataFrame({'interval':interval,
                         'genename':genename,
                         'ensid': ensid,
                         'genome': genome,
