...
outputpath = "./scmaui_pbmc_rna_cond_v1"

datapaths = ["data/gtxcombined.h5ad"]

adatas = load_data(datapaths, ["gtx"])
# dataset = SCMauiDataset(adatas, union=True)
dataset = SCMauiDataset(adatas, losses=["negbinom"], union=True, conditional=["sample"])
print(dataset)
...
ensemblevae.fit(dataset, epochs=200, batch_size=64)
