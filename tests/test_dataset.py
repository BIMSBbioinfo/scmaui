...
def test_paireddataset_intersect():
    ...
    assert dataset.shapes() == {'inputdims': [35300, 95482], 'outputdims': [35300, 95482]}
    assert dataset.adversarial_config()
    assert dataset.conditional_config()
...
def test_paireddataset_intersect_conditional():
    ...
    adatas = load_data([gtx, peaks], ['gtx', 'peaks'])
    dataset = SCDataset(adatas, losses=['mse']*2, union=False, conditional=['logreads', 'sample'])
    assert dataset.size() == 50
    assert dataset.modalities() == (['gtx', 'peaks'], ['gtx', 'peaks'])
    assert dataset.shapes() == {'inputdims': [35300, 95482], 'outputdims': [35300, 95482]}
    assert dataset.adversarial_config()
...
def test_paireddataset_union_conditional():
    ...
    adatas = load_data([gtx, peaks], ['gtx', 'peaks'])
    dataset = SCDataset(adatas, losses=['mse']*2, union=True, conditional=['logreads', 'sample'])
    assert dataset.size() == 150
    assert dataset.modalities() == (['gtx', 'peaks'], ['gtx', 'peaks'])
    assert dataset.shapes() == {'inputdims': [35300, 95482], 'outputdims': [35300, 95482]}
    assert dataset.adversarial_config()
...
def test_paireddataset_intersect_adversarial():
    ...
    adatas = load_data([gtx, peaks], ['gtx', 'peaks'])
    dataset = SCDataset(adatas, losses=['mse']*2, union=False, adversarial=['logreads', 'sample'])
    assert dataset.size() == 50
    assert dataset.modalities() == (['gtx', 'peaks'], ['gtx', 'peaks'])
    assert dataset.shapes() == {'inputdims': [35300, 95482], 'outputdims': [35300, 95482]}
    assert dataset.adversarial_config()
...
def test_paireddataset_union_adversarial():
    ...
    adatas = load_data([gtx, peaks], ['gtx', 'peaks'])
    dataset = SCDataset(adatas, losses=['mse', 'mse'], union=True, adversarial=['logreads', 'sample'])
    assert dataset.size() == 150
    assert dataset.modalities() == (['gtx', 'peaks'], ['gtx', 'peaks'])
    assert dataset.shapes() == {'inputdims': [35300, 95482], 'outputdims': [35300, 95482]}
    assert dataset.adversarial_config()
...
def test_covariates():
    ...
    adatas = load_data([gtx, peaks], ['gtx', 'peaks'])
    dataset = SCDataset(adatas, losses=['mse']*2, union=True, adversarial=['logreads', 'sample'])
...
def test_subset():
    ...
    assert subdataset.shapes() == {'inputdims': [35300, 95482], 'outputdims': [35300, 95482]}
