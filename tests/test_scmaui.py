...
from scmaui.utils import softmax1p, softmax1p0
from scmaui.utils import SparseSequence
from scmaui.utils import ScalarBiasLayer
from scmaui.utils import PaddingLayer
from scmaui.utils import AddBiasLayer
from scmaui.utils import load_data
from scmaui.utils import VAE, HyperVAE, CustomTuner
from scmaui.utils import create_flat_encoder, create_decoder
...
def test_scmaui_flat_saveload(tmpdir):
    (x_train, y_train), (x_test, y_test) = load_data('mnist')
    params = {
      'nhidden_e_0': 32,
      'nhidden_e_1': 32,
...
def test_hyperscmaui_flat(tmpdir):
    (x_train, y_train), (x_test, y_test) = load_data('mnist')
    biases = x_train.sum(0)
    biases /= biases.sum()

    build_model = HyperVAE('scmaui-flat', x_train.shape[-1], 3, biases)
...
def test_scmaui_flat_saveload_callback(tmpdir):
    (x_train, y_train), (x_test, y_test) = load_data('mnist')
    params = {
      'nhidden_e_0': 32,
      'nhidden_e_1': 32,
