...
class VAE(tf.keras.Model):
    ...
    def __init__(self, encoder, decoder, **kwargs):
        ...
        self.encoder_mean = Encoder(tf.keras.Model([inp for inp  in self.encoder.inputs if 'advinput_' not in inp.name],
                                    self.encoder.get_layer('z_mean').output))

        self.decoder = decoder

        self.encoder_params = tf.keras.Model([inp for inp  in self.encoder.inputs if 'advinput_' not in inp.name],
                                          self.encoder.get_layer('random_latent').output).trainable_weights
        self.batch_params = [w for w in encoder.trainable_weights if 'advnet_' in w.name]
        Ntotal = sum([np.prod(w.shape) for w in encoder.trainable_weights])
        Nencoder = sum([np.prod(w.shape) for w in self.encoder_params.trainable_weights])
