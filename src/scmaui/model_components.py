...
def create_decoder_base(params):
    ...
    masks = tuple([keras.Input(shape=(1,), name='target_mask_'+name) for name in mode_names])
    ...
    # conditional inputs
    conditional = tuple([keras.Input(shape=(ncat,),
                                     name='cond_'+name) for name, ncat \
                                     in zip(params['conditional_name'],
                                            params['conditional_dim'])])

    # adversarial inputs
    adversarial = tuple([keras.Input(shape=(ncat,),
                         name='adv_'+name) for name, ncat \
                         in zip(params['adversarial_name'],\
                                params['adversarial_dim'])])
...
def adversarial_predictor(layer, mask, targets, params, idx):
    ...
    pred_targets = [layers.Dense(dim, activation=_getlabel(ptype),
                                 ...
                                 for dim, ptype,bname in \
                                 zip(params['adversarial_dim'],
                                     params['adversarial_type'],
                                     params['adversarial_name'])]

    
    losses = [get_loss(t, mask, p, ptype) for t, p, ptype in \
              zip(targets, pred_targets, params['adversarial_type'])]
