    ...
    for label in labels:
        for adata in adatas:
            ...
            if is_numeric_dtype(adata.obs[label]):
                ...
                traindata = [[c] for c in adata.obs[label].astype(str).tolist() if not pd.isna(c)]
                #print(categories)
...
def _make_ds(x):
    ...
    if issparse(x):
        ds = tf.data.Dataset.from_tensor_slices(to_sparse_tensor(x)).map(lambda y: tf.cast(tf.sparse.to_dense(y), tf.float32))
    else:
...
def to_dataset(X, advlabels=None, condlabels=None, Y=None, batch_size=64, shuffle=True):
    ...
    if advlabels is not None and len(advlabels) > 0:
        ds_advlabel = tf.data.Dataset.zip(tuple([_make_ds(label) for label in advlabels or []]))
        ds_advlabels += (ds_advlabel,)

    ds_condlabels = ()
    if condlabels is not None and len(condlabels) > 0:
        ds_condlabel = tf.data.Dataset.zip(tuple([_make_ds(label) for label in condlabels or []]))
...
def _unionize(adata):
    ...
    for ada in adata['output']:
        ...
        if issparse(xnew):
            xnew = xnew.tocsr()
        new_output.append(AnnData(xnew,
                                  obs=df,
                                  var=ada.var,
                                  uns=ada.uns,
                                  obsm={'mask': X.dot(ada.obsm['mask'])}))
...
class SCDataset:
    ...
    def training_data(self, batch_size=64,
        ...
        advlabel_train, advlabel_test = self._split_train_test_data(advlabel, validation_split)
        condlabel_train, condlabel_test = self._split_train_test_data(condlabel, validation_split)
    ...
    def _get_input_data(self, adata, dummy_labels=False):
        X, mask_x, _, _, _, condlabel, _, _ = self._get_input_output_data(adata, dummy_labels=False)
    ...
    def _get_input_output_data(self, adata, dummy_labels=False):
        ...
        advlabels, advdtype = self._get_label(adata['input'], self.adversarial, dummy_labels=dummy_labels)
        condlabels, conddtype = self._get_label(adata['input'], self.conditional, dummy_labels=dummy_labels)
        return x_data, mask_x, y_data, mask_y, advlabels, condlabels, advdtype, conddtype
...
if __name__ == '__main__':
    adafile = ['/local/wkopp/scmaui_mskcc/10xgenomics/data/gtx.h5ad', '/local/wkopp/scmaui_mskcc/10xgenomics/data/peaks.h5ad']
    adatas = load_data(adafile, ['gtx', 'peaks'])


    dataset = SCDataset(adatas, adversarial=['logreads','sample'], conditional=['logreads','sample'])
    print(dataset)
    ...
    adatas = load_data(adafile, ['gtx', 'peaks'])
    adatas['input'][0] = AnnData(adatas['input'][0].X, var=adatas['input'][0].var, obsm={'mask': np.ones((adatas['input'][0].shape[0],1))},
